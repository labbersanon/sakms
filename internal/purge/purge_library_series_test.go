...
func TestScanLibrarySeries_ProposesOnlySeriesMatchingATagsRule(t *testing.T) {
	...
	got, err := ScanLibrarySeries(ctx, libStore, []pruning.Rule{
		{Name: "Flagged", Mode: string(mode.Series), Tags: []string{"BDSM"}, Enabled: true},
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestScanLibrarySeries_NoRulesMatchNothing(t *testing.T) {
	libStore := newTestLibraryStore(t)
	ctx := context.Background()
