...
## 2026-07-31 — Grabs section duplicate rows: idempotency guard on grab creation

**Problem:** Wade reported "in the grabs section there are duplicates." Routed through `ralph` per this project's mandatory troubleshooting rule.

**Root cause (confirmed via live production DB, read-only query on server1):** `grabs.Store.Create` did an unconditional INSERT with no existence check, and the schema (`migrations/0035_grabs_download.sql`) has no UNIQUE constraint on any grab-identity column. Live evidence: 3 `grabs` rows shared one torrent `download_gid` (infohash), created ~25-77 seconds apart — the download client had already deduped the physical torrent to one download, but sakms recorded three tracking rows for it. Secondary consequence, part of the same bug: `Store.GetByDownloadGID` is a first-row-only `QueryRow`, so only ONE of the duplicate rows ever gets marked `imported` on completion — the other two are permanently stranded at `queued`.

**Fix:** new `grabs.Store.ActiveByDownloadGID` (excludes only terminal `imported`/`failed` statuses, so a legitimate re-grab of something already completed or failed is never blocked), called before `Create` at all four grab-dispatch call sites (`autograb.go`'s main and direct-enclosure paths, `autograb_batch.go`'s bulk path, `search.go`'s manual grab). A DB-level UNIQUE constraint was deliberately deferred — the two pre-existing stranded production rows (ids 4 and 5) would make a partial-unique-index migration fail at deploy time, so that's a separate, explicit follow-up (reconcile the stranded rows first, then add the constraint), not silently skipped.

**Caught during review (architect REJECT → fix → APPROVE):** the first pass only fixed the backend — the new "already grabbing this release" response (`grabbed:false`, no fallback) had no frontend consumer, so `GrabDialog`'s render Switch matched neither of its two existing branches and rendered a blank modal, the exact "silent no-op" the fix exists to prevent. Also caught: the bulk-grab endpoint folded a duplicate into its existing `grabbed=true` outcome, inflating the batch's grabbed count and violating this feature's three-state honesty convention. Both fixed: `GrabDialog` gained a fallback Switch branch rendering the message; the batch result DTO gained a distinct `AlreadyGrabbing` outcome end-to-end (DTO → handler → `BulkResultModal`).

**Outcome:** regression tests lock exact row counts for both the duplicate-GID case (blocked) and the distinct-GID case (both created, proving the guard isn't over-broad), terminal-state exclusion, empty-GID handling, and the new frontend render states. Two existing stranded production rows (ids 4, 5) are left untouched — fixing the bug prevents new duplicates, it does not retroactively repair already-stranded rows; that needs a separate, explicitly-approved cleanup step. Full `go build`/`vet`/`test -race` and `pnpm typecheck`/`build`/`test` clean, zero new failures beyond the pre-existing unrelated `internal/sysinfo` gap.

## 2026-07-31 — Requests exclusion, Downloads cancel-with-delete + bulk-select, global download pause

**Feature request from Wade:** add a remove/delete action to Requests, Downloads, and Grabs, where removing should also cancel/delete in the download client. Per this project's mandatory pipeline, ran a real `deep-interview` first (6 rounds incl. Round 0, 7.5% final ambiguity) rather than building directly from the initial ask.

**Scope resolved during the interview, several premises corrected before any code was written:**
- Investigation found Grabs is documented as intentionally read-only ("deliberately NOT a management screen... gains no bulk affordance") — a remove button there would silently reverse a settled decision. Asked directly; **Grabs stays read-only, out of scope.**
- Investigation found this app doesn't actually use qBittorrent/NZBGet as live download engines at all — `dispatchToDownloadClient` only ever routes to an in-process `anacrolix` torrent engine or a native NNTP usenet engine. Both already had working per-item Cancel/Pause/Resume; the qBittorrent/NZBGet client packages exist but aren't in the active dispatch path.
- Neither engine's existing Cancel deleted files (confirmed via code read: `torrentlib.Torrent.Drop()` doesn't delete data; usenet's Cancel had no file-cleanup at all) — "delete files too" was a real, unimplemented gap, not a misunderstanding.
- Requests is a pure computed/read-only aggregation with no persisted per-request row — "remove" needed new exclusion-list persistence, not deletion of an existing row.
- A "global pause" request (surfaced mid-interview) was clarified to mean pause all active downloads AND block any new download from starting — gated once at the shared `dispatchToDownloadClient` choke point (already shared by manual and auto-grab dispatch) rather than duplicated per call site.
- Whether Requests/Downloads currently show items in unexpected lifecycle states was explicitly deferred by Wade as a separate, unverified question — not addressed by this work.

**Executed via `/team`** (2 parallel workers split along the file-conflict boundary — backend vs. frontend — rather than by feature, since all three backend sub-areas converge on the same shared files): backend added `internal/excludes` (new exclusion persistence), extended both download engines' `Cancel` to delete files, added bulk-cancel/exclude-batch endpoints, and the global-pause gate; frontend added bulk multi-select (reusing the existing `useBulkSelection` pattern from Rename/Purge/Dedup) and remove/pause controls to both screens.

**Security review caught two real, exploitable HIGH-severity path-traversal vulnerabilities before deploy**, missed by the initial functional and code-quality reviews: (1) the usenet engine's per-download staging directory was keyed on the raw, attacker-controlled release name (indexer/NZB-supplied) — a release named `..` would resolve the directory to the *parent* of the staging root, and Cancel's `os.RemoveAll` would wipe that parent tree (database, secret key, sibling downloads, everything); (2) the torrent engine's file-deletion loop had no containment check on the file paths it deletes, and those paths come from raw, unsanitized torrent metadata (`anacrolix/torrent`'s `File.Path()`) — a malicious torrent declaring a file path with `../` components could cause deletion of an arbitrary file outside the staging directory. Both fixed: the usenet directory is now keyed on the download's unique GID (which can't contain path separators or `..`) instead of the release name; the torrent deletion loop now applies the same path-containment guard already used elsewhere in the same file. Two adversarial regression tests were added that fail against the pre-fix code and pass against the fix, proving the holes are genuinely closed rather than just plausible-looking. A separate, pre-existing, out-of-scope path-traversal issue (unsanitized yEnc filename on the usenet write path) was also found and flagged as a follow-up — not part of this diff, not fixed here.

**Also verified as safe, adversarially**: Cancel on an already-completed-and-imported download cannot delete the real relocated library file — `rename.Relocate` moves (not copies) via `os.Rename`, and the download engine never re-points its tracked file paths to the post-import library location, so a post-import Cancel is a safe no-op on a now-gone staging path. Proven by a dedicated test in both engines (`TestCancel_AfterImport_LeavesLibraryFileUntouched`), independently re-verified by the architect reviewer by tracing the actual relocation code rather than trusting the claim.

**Outcome:** three-round review (architect functional-completeness APPROVE, code-reviewer APPROVE with minor non-blocking notes, security-reviewer initial REJECT on the two HIGH findings above → fix round → final security APPROVE). Full `go build`/`vet`/`test` and `pnpm typecheck`/`build`/`test` (496 tests) clean, zero new failures beyond the pre-existing unrelated `internal/sysinfo` gap.

## 2026-08-01 — Library sidebar tab: Movies/Series browsing separated from Tag CRUD

Spec: `.omc/specs/deep-interview-library-sidebar-tab.md` (~10% ambiguity, PASSED). Plan: `.omc/plans/autopilot-impl-library-sidebar-tab.md`. Real finding from the interview: no dedicated library-browsing screen existed — `Tag.tsx` accidentally already served that role for Movies/Series (full tracked-catalog grid, client-side title search, poster cards) alongside its actual purpose (tag CRUD).

**New `frontend/src/screens/Library.tsx`** (own sidebar entry/icon, `/library` route) takes over tracked-catalog browsing for Movies/Series: the relocated title search, plus two new capabilities — genre filter over the existing `genres[]` field, and added-date sort backed by a newly-exposed `createdAt` field (`internal/api/tracked.go`, `internal/apidto/dto.go`, regenerated `internal/apidto/ts/dto.gen.ts`). **`Tag.tsx` narrows back to pure tag-CRUD** in the same change — its browsing grid deleted, `PosterCard`/`DetailPanel` moved to Library verbatim (same tag-editing `act()`/`addTag`/`removeTag` mechanics, now reached by drilling into an item from Library).

Adult mode is explicitly out of scope for Library and its wire response stays byte-identical — `createdAt` is deliberately left unpopulated for Adult, and `Tag.tsx`'s Adult-mode table (`:499-592`) is untouched. This surfaces a genuine contradiction inside the spec's own Acceptance Criteria: AC5 says "Tag no longer a standalone browsing entry point," AC7 says "Adult's table view completely unchanged" — but Adult's tag-CRUD table has always lived inside `Tag.tsx` and was never given a separate Library-equivalent screen, so `Tag.tsx` remains the *only* way to reach it. Resolved (Wade-confirmed) by keeping Tag as a direct sidebar entry point for all three modes rather than either breaking Adult's access or building it a redundant new table screen the spec never asked for; AC5 is satisfied for Movies/Series, which is what it was actually aimed at. See `CLAUDE.md`'s "Sidebar + section-tab redesign" section and `docs/ROADMAP.md` item 12 for the same reasoning recorded where a future session will look for it before "fixing" Tag's remaining sidebar entry.

Delivered as a 4-task split (A: backend `createdAt` exposure, independent; B: shell/sidebar wiring shipping a stub `Library.tsx`; C: Library screen + Tag narrowing as one combined move-refactor, depends on A; D: this doc pass, depends on C), sequenced per the plan's deployment-safety note so Task B's stub screen never reached `origin/main` on its own ahead of Task C.

**Outcome:** `go build ./... && go vet ./... && go test ./...` and `pnpm typecheck && pnpm test && pnpm build` all clean, 499/499 tests passing.

## 2026-08-01 — Connections tab eliminated; multi-connection registry; multi-subscription Usenet; the first unattended auto-grab

Spec: `.omc/specs/deep-interview-connections-elimination-usenet-multisub.md` (12% ambiguity, PASSED). Plan: `.omc/plans/autopilot-impl-connections-elimination.md` — Critic-reviewed and revised four times before any executor ran, then built as a 4-wave, 12-task autopilot split (Wave 0 parallel foundations → Wave 1a solo signature change → Wave 1b API/handlers/DTOs/frontend → Wave 2 schedulers + screens → Wave 3 tests, then this doc pass).

**The registry is a split, not a replacement — this was the plan's single biggest scope finding.** The spec's Goal 5 ("the registry replaces `PRIMARY KEY(service)`") and its own constraint that TMDB/TVDB/StashDB/FansDB/TPDB/Trakt/AI stay single-connection reconcile exactly one way. Migration `0053` adds a **new** `service_connections` table (plus `service_connection_modes` for many-to-many mode assignment) and moves exactly **two** rows out of `connections` — `nntp` and `jellyfin`. `internal/connections` and its table survive unchanged for the ~7 genuinely-singleton services. New package `internal/serviceconn`, deliberately not named `connections` (two same-named things would be a permanent readability trap), modelled on `internal/rssfeeds`' store shape including the three-state secret rule. The Down migration is deliberately lossy and says so in-file: the old `PRIMARY KEY(service)` shape cannot represent multiple rows per provider, which is the entire point of the feature. Ciphertext is copied verbatim (same `internal/secrets` key, no decrypt/re-encrypt step); the one thing SQLite could not do — parsing the legacy `nntp://host:port` URL into host/port/tls — is an idempotent Go boot step (`serviceconn.BackfillUsenetURL`).

**Settings' Connections tab is gone**, its ~11 services individually redistributed rather than bulk-grouped (every placement operator-confirmed): Prowlarr and Stash to a new Advanced → **API Connections** section, the metadata/catalog services into Library, Trakt to its own card, AI unchanged, and Usenet to its own dedicated tab. Stash sits with Prowlarr rather than beside StashDB/FansDB/TPDB despite the obvious functional adjacency — an individually confirmed placement decision, recorded in `APISection.tsx`'s header so nobody "improves" the grouping later. `PUT`/`DELETE /api/connections/{service}` now reject `nntp` and `jellyfin` outright (`rejectMovedConnectionService`) instead of silently writing to a table nothing reads anymore, and `internal/api/setup.go` was updated in the same change — it would otherwise have broken silently the moment migration 0053 ran.

**Real Emby and Plex clients** (`internal/emby`, `internal/plex`) now sit alongside Jellyfin, each with per-connection mode assignment (many-to-many, no exclusivity) replacing the hardcoded `Jellyfin = Movies/Series` scoping; `mode.Session` gained a `Players` fan-out behind a `PlayerNotifier` interface. Both new packages carry the house "unverified assumptions" honesty note in their package docs: Emby's shapes come from its published OpenAPI spec (fetched and inspected, wire contract verified, live runtime behaviour not), Plex's from community reverse-engineered documentation because no official field-level JSON reference exists. Emby's auth deliberately uses `X-Emby-Token` rather than Jellyfin's `Authorization: MediaBrowser` scheme — the two servers diverged on exactly that point after the 2018 fork. **Stash was deliberately NOT folded into the player registry**: its notify path splits Created/Modified → `RescanPaths` vs Deleted → `CleanMetadata`, which `mode.go` itself calls the single most important correctness guardrail in that feature, and a uniform interface would dilute or hide the split. Stash stays a singleton `connections` row with its existing hardcoded Adult path.

**Usenet is genuinely multi-subscription.** `usenet.Manager` grew a pool set (one per enabled subscription) and `SetSubscriptions`, making the engine runtime-reconfigurable — previously it was a boot-time singleton constructed once, so adding a subscription in the UI did nothing until process restart. It is now built unconditionally (never nil; callers ask `HasSubscriptions()` instead of nil-checking), and the global `downloader_max_connections` setting was narrowed to **torrent-only** since each subscription now carries its own `MaxConns`. The new Settings → Usenet page documents two things it must never grow: a priority/fallback ordering field, and a retry-interval input.

**A documented spec deviation, recorded here as well as in `CLAUDE.md` because a future session will look in either place: "all configured subscriptions are queried in parallel" was reinterpreted as a RETRIEVAL-stage fan-out, not a search-stage one.** Read literally, the spec asks each Usenet subscription to return searchable results to be scored. That is not implementable: **NNTP has no search verb.** An NNTP server offers `ARTICLE`/`BODY` retrieval by message-ID and nothing else, and header enumeration is an indexer-scale operation — which is exactly what Prowlarr already is. Neither existing scorer has a single field an NNTP server could populate (both take release metadata: title, size, resolution, codec, seeders, publish date). What was built is a two-stage pipeline: **one** Prowlarr search produces usenet candidates → `autograb.Select` scores them and picks a winner or refuses → the winner's NZB is parsed to segments → **the fan-out across subscriptions happens at segment retrieval.** That is also the real-world reason operators run multiple providers: differing retention and takedown coverage, i.e. article *availability* — a retrieval concern, not a search one. Nothing in the spec is lost under this reading: "no priority ordering" holds (no priority field, no reorder UI, no ranked-chain concept anywhere); "ranked via the codebase's existing scoring mechanism" holds (`autograb.Select`, an existing scorer — no new one was written, per the spec's own non-goal); "highest-scored candidate auto-downloads with no human review step" holds; "no match → stays pending, retries every 24h" holds at *either* stage, since no qualifying candidate and no subscription holding the articles both park the same `pending_retry` row; and "retry never bypasses scoring" holds because a retry restarts at the Prowlarr search rather than re-dispatching a cached winner. One precision worth recording, since an in-code comment overstates it: per-segment probing is *sequential fallback* across pools (`fetchSegmentAny`), deliberately — probing every server simultaneously for the same article would download N copies of one body for N times the bandwidth and connection consumption. The concurrency is across *segments*, bounded by an `errgroup` limited to the summed `MaxConns` of all enabled subscriptions, so multiple subscriptions genuinely do serve one download at once and every enabled subscription is tried for every segment.

**The staged-for-approval exception, and why it needed its own set of bounds.** This ships the **third** documented reversal of the staged-for-approval convention, after 2026-07-17's bulk-apply and 2026-07-24's bulk-grab — and it is structurally different from both. Bulk-apply acts on already-reviewed Pending proposals (a review step precedes the batch); bulk-grab has no staging step but the operator's own click *is* the approval. **This one has no human action in the loop at all**: when the 24h retry cycle fires, nobody clicked anything and a release can be dispatched to a download client on the system's own initiative. Four bounds replace the human, all enforced in code: (1) an off-by-default global toggle, `usenet_autograb_enabled`, whose gate is enforced in exactly one place — inside `RunAutoGrab` (`internal/api/autograb_shared.go`) — so no trigger can route around it, with Discover's shipped one-click Grab (`TriggerOperator`) the deliberate ungated exemption since its click is still the approval; (2) a qualification predicate that refuses to grab an ungradeable release — scoring uses `autograb.Select` rather than `release.ScoreCandidate` precisely because only the former can express "nothing qualified", so a candidate with unknown size or runtime is never dispatched; (3) no grab without a scored match under any condition including retry, because every trigger goes through the one `RunAutoGrab` path and a retry re-runs the whole pipeline from the search; (4) a permanent-failure path that never retries — a 451 `ErrArticleRemoved` (takedown) maps straight to `failed`, only a 430 from *every* configured subscription is retryable, and retries are capped at `maxRetryAttempts = 5` after which the row becomes `failed` with an explaining reason, so a permanently ungradeable item terminates rather than being worked forever. Full amendment text lives in `CLAUDE.md` under Established engineering conventions → Staged-for-approval.

**`CLAUDE.md`'s AMENDED 2026-07-23 scheduler note is corrected, not extended.** That note claimed "every scheduler in it is Scan/propose/passive-flag only — nothing is auto-Applied." That is now false: `RunUsenetRetry` (`internal/api/usenetretry.go`) is the first scheduler in this codebase that **dispatches downloads** rather than proposing work for review. Its two jobs per cycle are the authoritative sweep converting asynchronous usenet retrieval failures into `pending_retry` (430) or `failed` (451) — authoritative because `checkImportHandler` only runs when a browser polls it, so on a headless instance nothing else would ever move a failed retrieval out of "downloading" — and the re-search of every due row. Its interval is written server-side as a coupled side effect of the auto-grab toggle (on → 86400, off → 0) rather than being an independent knob, precisely so the two can never disagree. The same correction also fixes a stale enumeration: there are **six** interval-driven schedulers live today, not four — the 2026-07-23 note omitted `internal/parseentity` entirely, and `main.go`'s own launch comments ("the fourth", "the fifth") inherit that omission and are not authoritative.

**Two findings the plan did not predict, both corrections to it rather than to the spec:**
- **The plan's Lossless-bypass claim was wrong, and the truth is more bounded.** §2.3.1 predicted that on the toggle-ON Search hook "only a Lossless-source release can auto-grab," since a query-only search carries no runtime. In fact `autograb.GradeCandidate` short-circuits on `SizeBytes <= 0 || RuntimeSeconds <= 0` *before* reaching the Lossless remux/bluray bypass, so **the Search hook can never auto-grab at all** — every toggle-ON search parks a `pending_retry` row. Reported rather than "fixed" by inventing a title→TMDB resolution step inside the hook, which would have been a scope decision disguised as a bug fix. The consequence is worth carrying forward: the rows this feature can genuinely convert into an unattended dispatch are the ones the GID sweep parks (real dispatched grabs with a real TMDB id and runtime); for Search-hook rows the retry loop provides *termination* via the attempt cap, not eventual success.
- **A duplicate-grab bug the plan never anticipated.** A successful retry that created a *fresh* grabs row would leave the original still `pending_retry` with a `retry_after` in the past, so the next cycle would grab the same release again — and because `AddNZB` mints a fresh GID per call, the existing GID dedup guard could not catch it. Fixed by having a retry **re-arm its own row** (`AutoGrabRequest.ExistingGrabID` → new `grabs.Relaunch`) instead of recording a second one. **This is a distinct bug from the 2026-07-31 "Grabs: idempotency guard" entry above** (`ActiveByDownloadGID`, which blocks two rows sharing one GID) — different cause, different mechanism, and neither subsumes the other.

**Also shipped:** the retry state itself (migration `0054` adds `download_url_encrypted`, `retry_after`, `retry_count`, `retry_reason` to `grabs` — the download URL is encrypted at rest for the same reason RSS feed URLs are, since an indexer/NZB URL commonly embeds an API key, and carries `json:"-"` so it can never reach a wire DTO); a `pending_retry` status that Requests renders as a real, explainable state with a human countdown blurb ("Retrying in 6h — no candidate cleared the quality floor") rather than a silent disappearance; the shared toggle-gated search branch `runToggleGatedSearch` living in **one** function called from both `searchHandler` (Movies/Series) and `adultSearchHandler` (Adult), never two copies; and an operator-exclusion check in the retry loop so a title the operator removed from the Requests worklist is skipped rather than re-searched — a scheduler overriding an explicit human decision would defeat the point of the bounds above.

**Outcome:** this final task was documentation-only (this entry, plus `CLAUDE.md`'s two amendments and `docs/ROADMAP.md` item 3) and changed no code. What was verified in this pass specifically, and nothing more: `go build ./...` and `go vet ./...`, both green. Per-wave test verification belongs to each implementation task and is recorded in `.omc/plans/autopilot-impl-connections-elimination.md`; it was not re-run here, and is deliberately not asserted in this entry.

## 2026-08-01 — Storage Allocation Breakdown by media type and quality tier

Spec: `.omc/specs/deep-interview-storage-allocation-breakdown.md` (~9% ambiguity, PASSED). Plan: `.omc/plans/autopilot-impl-storage-allocation-breakdown.md`.

**Corrected assumption, documented before implementation began**: the spec's predecessor ROADMAP entry claimed "using data already tracked in `internal/library`/`internal/quality`, no new collection needed" — verified against live code to be false. No per-file size and no per-item quality-tier attribute exist anywhere in the schema prior to this work. This was genuine new instrumentation (schema + capture hooks + backfill job), not a query-only feature.

**New Dashboard section** — `frontend/src/screens/Dashboard.tsx` gained a Storage Allocation Breakdown table (Movies/Series/Adult rows × quality-tier columns, including "Unknown"), each cell showing total size (on a B–TB unit ladder, not a flat GB figure — a plain GB readout was rejected as too coarse for a multi-TB library) and item count. Movies/Series cells drill down into a filtered item view in the Library sidebar tab (which shipped earlier the same day); Adult cells are informational only. Note: a Series row's *total* item count can exceed its true series count, since a series spanning two quality tiers is counted once per tier cell (each cell's own count still reconciles with that cell's drill-down).

**Schema changes** — migration `0055_library_size_tier.sql` adds two columns to `library_items`, `library_episodes`, and `library_scenes`:
  - `size` (bytes) — captured via `os.Stat` at Apply/Grab time going forward
  - `quality_tier` (text, one of low/medium/high/lossless/unknown) — captured at grab/relocate time from **the mode's currently-configured quality tier setting** (the same value `autoGrabTier` already reads for search scoring), **not** `grabs.quality_profile_id`. That field turned out to be dead code — written only from a frontend request field zero callers ever send (confirmed via grep, zero hits), so every existing row has it at `0` with no defined mapping onto a tier anyway.

**Boot-time backfill job** — a background goroutine launched after the HTTP server starts listening (not before — a stale/disconnected CIFS/NFS mount doing a per-item `os.Stat` could otherwise hang boot). Idempotent via a per-row `WHERE id = ? AND size = 0 AND quality_tier = ''` guard on every UPDATE, which also protects against clobbering a fresher concurrent write from a live Apply. For every existing tracked item:
  - **Size**: `os.Stat` on the stored `FilePath` (if file exists, the size is real; if deleted, stays 0)
  - **Tier**: **2-step** fallback chain — the spec's original design called for a 3-step chain starting with a `grabs`-table record match, but that step was dropped as dead code for the same `quality_profile_id` reason above (a `grabs` row carries no usable quality signal). The real chain: (1) filename inference via `release.Parse()` against the file's path **relative to its root folder** (not just the basename — SAK's renamer strips quality tokens from filenames, but a season-pack parent directory can still carry them) + a new `quality.InferTier` reverse-lookup against `internal/quality`'s tier definitions, (2) explicit "unknown" tier bucket — an accepted, honestly-labelled outcome, not a bug.

**Backend endpoint** — `GET /api/admin/storage-allocation` returns a grouped SQL query (no per-request disk I/O or external calls), a dense 3×5 grid:
  ```
  { rows: [{ mode: "movies"|"series"|"adult", cells: [{ tier, totalBytes, itemCount }], rowTotalBytes, rowItemCount }], tiers: [...], grandTotalBytes }
  ```
  Sizes are pre-computed and stored, not calculated on-demand. Series' `itemCount` counts distinct series (not distinct episode files), so it reconciles with what the Library drill-down actually lists.

**Two accepted limitations** (non-blocking, documented in ROADMAP and code comments):
  1. **Well-Renamed libraries backfill mostly to "unknown" tier.** SAK's renamer strips quality tokens from filenames during relocation, so only un-Renamed/freshly-grabbed files (or season-pack parent directories) carry inferable quality tokens for the filename-inference fallback. This is the spec's own explicitly-accepted outcome.
