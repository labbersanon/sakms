...
func TestTitleQualityPrefs_RoundTripAndDelete(t *testing.T) {
	...
	if !got.HasOverride || !got.MaxResolutionSet || got.MaxResolution != 1080 {
		t.Fatalf("got %+v", got)
	}
	...
	got, err = s.GetTitleQualityPrefs(ctx, mode.Series, 4589)
	if err != nil {
		t.Fatalf("get after clear max: %v", err)
	}
	if got.MaxResolutionSet {
		t.Fatalf("max should inherit, got set=%v val=%d", got.MaxResolutionSet, got.MaxResolution)
	}
