...
// TestListTPDBSceneAndMovie_FiltersToTPDBScenesAndMovies covers the
// temporary performers-backfill support method: only tpdb-sourced Scene/
// Movie rows come back — a Studio/Performer row (no Grab/performers concept)
// and a non-tpdb (stashdb) row are both excluded.
func TestListTPDBSceneAndMovie_FiltersToTPDBScenesAndMovies(t *testing.T) {
	s := newTestReleaseStore(t)
	ctx := context.Background()

	releases := []MatchedRelease{
		{RowType: RowScene, EntityID: "1", EntitySource: "tpdb", EntityTitle: "TPDB Scene"},
		{RowType: RowMovie, EntityID: "2", EntitySource: "tpdb", EntityTitle: "TPDB Movie"},
		{RowType: RowScene, EntityID: "3", EntitySource: "stashdb", EntityTitle: "StashDB Scene"},
		{RowType: RowStudio, EntityID: "4", EntitySource: "tpdb", EntityTitle: "TPDB Studio"},
	}
	for _, r := range releases {
		if err := s.Insert(ctx, r); err != nil {
			t.Fatalf("unexpected error: %v", err)
	...
	got, err := s.ListTPDBSceneAndMovie(ctx)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(got) != 2 {
		t.Fatalf("expected 2 tpdb scene/movie entities, got %d: %+v", len(got), got)
	}
	titles := map[string]bool{}
	for _, m := range got {
		titles[m.EntityTitle] = true
	}
	if !titles["TPDB Scene"] || !titles["TPDB Movie"] {
		t.Errorf("expected TPDB Scene and TPDB Movie, got %+v", got)
	}
}

// TestUpdatePerformers_OverwritesAndSurvivesRoundTrip covers the temporary
// performers-backfill support method: an update is visible via List, and
// updating a nonexistent entity is a no-op, not an error (matching this
// package's Delete-style convention).
func TestUpdatePerformers_OverwritesAndSurvivesRoundTrip(t *testing.T) {
	s := newTestReleaseStore(t)
	ctx := context.Background()

	if err := s.Insert(ctx, MatchedRelease{RowType: RowScene, EntityID: "1", EntitySource: "tpdb", EntityTitle: "Scene"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if err := s.UpdatePerformers(ctx, RowScene, "tpdb", "1", []string{"Jane Doe"}); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if err := s.UpdatePerformers(ctx, RowScene, "tpdb", "does-not-exist", []string{"Nobody"}); err != nil {
		t.Fatalf("expected updating a nonexistent entity to be a no-op, got error: %v", err)
	}
	...
	if len(list) != 1 || len(list[0].Performers) != 1 || list[0].Performers[0] != "Jane Doe" {
		t.Errorf("expected performers to be updated, got %+v", list)
	}
}
