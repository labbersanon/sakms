...
// TestAIConfirmAdultReleases directly covers aiConfirmAdultReleases — the
// 2026-08-11 mandatory confidence gate that replaced the studio+title query
// cascade in autoGrabSearch's mode.Adult branch (see that function's doc
// comment for the full revision history). Unlike aiEscalateTitleMatch
// (FilterReleases' zero-match-only rescue), this always runs and requires
// BOTH studio and title to independently clear adultConfidenceFloor.
func TestAIConfirmAdultReleases(t *testing.T) {
	t.Run("AI confirms both studio and title — kept", func(t *testing.T) {
		ai := &fakeReleaseMatchAI{fn: func(prompt string) (map[string]any, error) {
			return map[string]any{"studio": "Cathy's Craving", "title": "Teasing Cheerleader", "performers": []any{}}, nil
		}}
		releases := []prowlarr.Release{
			{GUID: "1", Title: "CathysCraving.Teasing.Cheerleader.XXX.1080p-GROUP"},
		}
		got := aiConfirmAdultReleases(context.Background(), releases, "Cathy's Craving", "Teasing Cheerleader", ai)
		if len(got) != 1 {
			t.Fatalf("expected the release to be confirmed, got %+v", got)
		}
	})

	t.Run("title matches but AI-parsed studio doesn't — dropped", func(t *testing.T) {
		ai := &fakeReleaseMatchAI{fn: func(prompt string) (map[string]any, error) {
			return map[string]any{"studio": "A Totally Different Studio", "title": "Teasing Cheerleader", "performers": []any{}}, nil
		}}
		releases := []prowlarr.Release{
			{GUID: "1", Title: "OtherStudio.Teasing.Cheerleader.XXX.1080p-GROUP"},
		}
		got := aiConfirmAdultReleases(context.Background(), releases, "Cathy's Craving", "Teasing Cheerleader", ai)
		if len(got) != 0 {
			t.Fatalf("expected a same-titled release from an unrelated studio to be dropped, got %+v", got)
		}
	})

	t.Run("AI declines to guess (empty title) — dropped, not guessed at", func(t *testing.T) {
		ai := &fakeReleaseMatchAI{fn: func(prompt string) (map[string]any, error) {
			return map[string]any{"studio": nil, "title": nil, "performers": []any{}}, nil
		}}
		releases := []prowlarr.Release{
			{GUID: "1", Title: "some-garbled-release-name"},
		}
		got := aiConfirmAdultReleases(context.Background(), releases, "Cathy's Craving", "Teasing Cheerleader", ai)
		if len(got) != 0 {
			t.Fatalf("expected an AI-declined candidate to be dropped, got %+v", got)
		}
	})

	t.Run("a per-candidate AI error drops only that candidate", func(t *testing.T) {
		ai := &fakeReleaseMatchAI{fn: func(prompt string) (map[string]any, error) {
			if strings.Contains(prompt, "bad-release") {
				return nil, fmt.Errorf("simulated AI failure")
			}
		...
		releases := []prowlarr.Release{
			{GUID: "1", Title: "bad-release-XYZ"},
			{GUID: "2", Title: "CathysCraving.Teasing.Cheerleader.XXX.1080p-GROUP"},
		}
		got := aiConfirmAdultReleases(context.Background(), releases, "Cathy's Craving", "Teasing Cheerleader", ai)
		if len(got) != 1 || got[0].GUID != "2" {
			t.Fatalf("expected only the second (successfully-confirmed) candidate to survive, got %+v", got)
		}
	})

	t.Run("nil AI client degrades to title-only similarity — high enough, kept", func(t *testing.T) {
		releases := []prowlarr.Release{
			{GUID: "1", Title: "CathysCraving.Teasing.Cheerleader.XXX.1080p-GROUP"},
		}
		got := aiConfirmAdultReleases(context.Background(), releases, "Cathy's Craving", "Teasing Cheerleader", nil)
		if len(got) != 1 {
			t.Fatalf("expected title-only similarity to confirm the release with no AI configured, got %+v", got)
		}
	})

	t.Run("nil AI client degrades to title-only similarity — no overlap, dropped", func(t *testing.T) {
		releases := []prowlarr.Release{
			{GUID: "1", Title: "Completely.Unrelated.Release.Name.2020-GROUP"},
		}
		got := aiConfirmAdultReleases(context.Background(), releases, "Cathy's Craving", "Teasing Cheerleader", nil)
		if len(got) != 0 {
			t.Fatalf("expected an unrelated release title to be dropped with no AI configured, got %+v", got)
		}
	})
}
