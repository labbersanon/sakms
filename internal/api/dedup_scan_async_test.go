...
func TestDedupScan_ConcurrentSameMode_Returns409(t *testing.T) {
	...
	hub := dedupscan.New()
	srv := httptest.NewServer(NewMux(testHTTPClient(), connStore, nil, propStore, allowStore, testProber(t), testPHasher(t), testVideoHasher(t), settingsStore, grabsStore, libStore, slidersStore, traktStore, adultNewestRowStore, adultNewestReleaseStore, testFeedHealth(), rssFeedsStore, nil, nil, nil, nil, hub, nil))
	defer srv.Close()
...
func TestDedupScan_DoneEventArrivesOnStream(t *testing.T) {
	...
	hub := dedupscan.New()
	srv := httptest.NewServer(NewMux(testHTTPClient(), connStore, nil, propStore, allowStore, prober, testPHasher(t), testVideoHasher(t), settingsStore, grabsStore, libStore, slidersStore, traktStore, adultNewestRowStore, adultNewestReleaseStore, testFeedHealth(), rssFeedsStore, nil, nil, nil, nil, hub, nil))
	defer srv.Close()
