...
func TestDedupWorkflow_Series_ScanThenApply_EndToEnd(t *testing.T) {
	...
	srv := httptest.NewServer(NewMux(testHTTPClient(), connStore, nil, propStore, allowStore, prober, testPHasher(t), testVideoHasher(t), settingsStore, grabsStore, libStore, slidersStore, traktStore, adultNewestRowStore, adultNewestReleaseStore, testFeedHealth(), rssFeedsStore, nil, nil, nil, nil, dedupscan.New(), nil))
	defer srv.Close()
...
func TestDedupWorkflow_Series_SeasonPack_ScanFindsGroupedDuplicate(t *testing.T) {
	...
	srv := httptest.NewServer(NewMux(testHTTPClient(), connStore, nil, propStore, allowStore, prober, phasher, testVideoHasher(t), settingsStore, grabsStore, libStore, slidersStore, traktStore, adultNewestRowStore, adultNewestReleaseStore, testFeedHealth(), rssFeedsStore, nil, nil, nil, nil, dedupscan.New(), nil))
	defer srv.Close()
