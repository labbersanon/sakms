...
// Decision: retire: table absent in baseline by design
// Archive: internal/db/migrations_sqlite_archive/
// Review if: baseline regenerates and this assertion still belongs in app code.

func TestRetired_adult_merged_row_cache(t *testing.T) {
	t.Skip("retired: retire: table absent in baseline by design")
}
